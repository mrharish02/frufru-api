...
app.get("/updateAllocationSettings", async (c) => {
  ...
  try {
    const result = await c.env.DB.prepare("SELECT * FROM AllocationSettings WHERE SID = ?;").bind(sid).all();
...
app.post("/updateAllocationSettings", async (c) => {
  ...
  try {
    ...
    if (SID === void 0 || BoxCount === void 0 || CutOffTime === void 0 || PriorityStockDisposal === void 0 || DuplicateOrderAlert === void 0 || SizeAllocationRatioAlert === void 0 || RankAProductPrioritization === void 0 || RankAStrawberriesPrioritization === void 0 || CustomerRequestPrioritization === void 0) {
      return c.json({ message: "Missing required fields" }, 400);
    }
    const result = await c.env.DB.prepare(`INSERT INTO AllocationSettings (SID, BoxCount, CutOffTime, PriorityStockDisposal, DuplicateOrderAlert, SizeAllocationRatioAlert, RankAProductPrioritization, RankAStrawberriesPrioritization, CustomerRequestPrioritization) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(SID, BoxCount, CutOffTime, PriorityStockDisposal, DuplicateOrderAlert, SizeAllocationRatioAlert, RankAProductPrioritization, RankAStrawberriesPrioritization, CustomerRequestPrioritization).run();
    if (result.success) {
      return c.json({ message: "Data inserted successfully", result }, 200);
    } else {
...
app.put("/updateAllocationSettings", async (c) => {
  ...
  try {
    ...
    const result = await c.env.DB.prepare(`
      ...
      WHERE SID = ?
    `).bind(BoxCount, CutOffTime, PriorityStockDisposal, DuplicateOrderAlert, SizeAllocationRatioAlert, RankAProductPrioritization, RankAStrawberriesPrioritization, CustomerRequestPrioritization, SID).run();
    if (result.success) {
      return c.json({ message: "Data updated successfully", result }, 200);
    } else {
...
function countNullValues(orders) {
  let nullCount = 0;
...
__name(countNullValues, "countNullValues");
app.post("/uploadOrderData", async (c) => {
  ...
  if (file instanceof File) {
    let sheetToJsonUsingW2 = function(worksheet2) {
      const result = [];
      const range = utils.decode_range(worksheet2["!ref"]);
      for (let R = range.s.r + 1; R <= range.e.r; ++R) {
        const row = {};
        for (let C = range.s.c; C <= range.e.c; ++C) {
          const cellAddress = utils.encode_cell({ r: R, c: C });
          const headerAddress = utils.encode_cell({ r: range.s.r, c: C });
    ...
    var sheetToJsonUsingW = sheetToJsonUsingW2;
    __name(sheetToJsonUsingW2, "sheetToJsonUsingW");
    ...
    `).bind(fileName, uploadStatus, fileSize).run();
    ...
    const jsonData = sheetToJsonUsingW2(sheet);
    const orders = [];
    jsonData.forEach((row) => {
      const order = { SystemUploadDate: (/* @__PURE__ */ new Date()).toISOString(), OrderDate: row["Order Date"] || null, OrderNo: row["Order No"] || null, ProductCode: row["Product Code"] || null, Quantity: row["Quantity"] || null, CustomerCode: row["Customer Code"] || null, Message: row["Message"] || null, DesiredDeliveryDate: row["Desired Delivery Date"] || null, DesiredDeliveryTime: row["Desired Delivery Time"] || null, fileId };
      orders.push(order);
    });
    const insertPromises = orders.map((order) => {
      return c.env.DB.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(order.SystemUploadDate, order.OrderDate, order.OrderNo, order.ProductCode, order.Quantity, order.CustomerCode, order.Message, order.DesiredDeliveryDate, order.DesiredDeliveryTime, "pending", order.fileId).run();
    });
    try {
...
app.get("/getUploadedOrders", async (c) => {
  ...
  try {
    const countResult = await c.env.DB.prepare(`SELECT COUNT(*) as count FROM orderdata`).first();
    if (!countResult || countResult.count === void 0) {
...
app.get("/getOrderData/:fileId", async (c) => {
  ...
  try {
    ...
    return c.json({ message: "Error fetching file", error }, 500);
  }
});
  ...
  try {
    ...
    for (const item of data) {
      const values = [
        ...
        item.apartmentName,
        item.customerRank,
        item.isAlternativeVariety,
        item.isAlternativeSize,
        item.prohibitedVarieties,
        item.prohibitedFarms,
        fileName
      ];
...
app.get("/getUploadedMasterData", async (c) => {
  ...
  const offset = (page - 1) * pageSize;
  try {
    const countResult = await c.env.DB.prepare(`SELECT COUNT(*) as count FROM masterdata`).first();
    if (!countResult || countResult.count === void 0) {
      return c.json({ message: "Error fetching total count" }, 500);
    }
    const totalCount = countResult.count;
...
app.get("/getMasterData/:fileName", async (c) => {
  ...
  try {
    const countResult = await c.env.DB.prepare(`SELECT COUNT(*) as count FROM masterdata  WHERE fileName = ?`).bind(file).first();
    if (!countResult || countResult.count === void 0) {
      return c.json({ message: "Error fetching total count" }, 500);
    }
    const totalCount = countResult.count;
...
app.get("/getUploadedMetaData", async (c) => {
  ...
  const offset = (page - 1) * pageSize;
  try {
    const countResult = await c.env.DB.prepare(`SELECT COUNT(*) as count FROM mastermetadata`).first();
    if (!countResult || countResult.count === void 0) {
      return c.json({ message: "Error fetching total count" }, 500);
    }
    const totalCount = countResult.count;
